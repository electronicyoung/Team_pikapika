...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def drop_flow(self, datapath, priority, match, table_id, buffer_id=None):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def Forward_table(self, datapath, priority, table_id):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def _packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            match2 = parser.OFPMatch(eth_type=0x0800, ip_proto=6, tcp_dst=80)
            # verify if we have a valid buffer_id, if yes avoid to send both
            # flow_mod & packet_out
            if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                self.drop_flow(datapath, 10, match2, CURRENT_TABLE, msg.buffer_id)
                self.add_flow(datapath, 0, match1, actions, FORWARD_TABLE, msg.buffer_id)
                self.Forward_table(datapath, 1, CURRENT_TABLE)
                return
            else:
                self.drop_flow(datapath, 10, match2, CURRENT_TABLE)
                self.add_flow(datapath, 0, match1, actions, FORWARD_TABLE)
                self.Forward_table(datapath, 1, CURRENT_TABLE)
